...
# Design: NIC1 bridged (WAN) e NIC2 host-only (LAN)
# Host (VPS):
#   - enp35s0: 136.243.208.128/32 (exclusivo do host)
#   - vboxnet0 (LAN host-only):        192.168.56.1/24
#   - Rotas no host (após subir CHR):
#       ip route add 136.243.208.128/29 via <IP_CHR_BRIDGED> dev <BRIDGE_IFACE>
#       ip -6 route add 2a01:48:171:76b::/64 via fe80::<LL_CHR_BRIDGED> dev <BRIDGE_IFACE>  # ou via global quando configurado
#   - sysctl: net.ipv4.ip_forward=1, net.ipv6.conf.all.forwarding=1, rp_filter=2
# CHR:
#   - ether1 (WAN bridged): <CONFIGURAR_IP_AQUI>/<CIDR>, default gw <GATEWAY_DA_REDE_BRIDGED>
...
# Permite gerenciamento apenas de redes internas (ether1 ou ether2)
/ip service set winbox disabled=no address=""
/ip service set ssh address="" port=22

# 1) Endereçamento IPv4
# ether1 = WAN (bridged) - **CONFIGURAR MANUALMENTE OU VIA DHCP CLIENT**
# /ip address add address=192.168.57.2/24 interface=ether1 comment=WAN-BRIDGED
# /ip route add dst-address=0.0.0.0/0 gateway=192.168.57.1 comment="Default route via bridged network"
...
#: /ipv6 route add dst-address=::/0 gateway=<HOST_BRIDGED_IPV6>

# 2.1) WireGuard (admin/túnel) — preencha chaves e peer (criar antes do firewall)
...
# Adicione aqui o IP ou sub-rede do host na rede bridged se for gerenciar por ela
# /ip firewall address-list add list=admin-allowed address=<HOST_IP_BRIDGED>/32 comment=Host-Bridged
/ip firewall address-list add list=admin-allowed address=136.243.208.128/32 comment=Host-Public
...
# Removida regra para Winbox via NAT PF (ether3)
